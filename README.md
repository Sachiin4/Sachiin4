<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=34&duration=2600&pause=900&color=58A6FF&center=true&vCenter=true&width=720&lines=Sachin+Mandpe" alt="Sachin Mandpe typing animation" />

<img src="https://readme-typing-svg.demolab.com?font=Pacifico&size=28&duration=2400&pause=900&color=F97316&center=true&vCenter=true&width=720&lines=AI+Engineer" alt="AI Engineer typing animation" />

### AI Engineer focused on practical AI agents, LLM applications, and Python automation systems.

I turn ideas into working prototypes, especially where AI can help with research, interviews, validation, decision-making, and business analysis.

</div>

---

## About Me

- Building multi-agent AI systems using Google ADK, Gemini, and MCP
- Creating LLM-powered research and utility assistants
- Working with Python automation, data workflows, and ML prototypes
- Exploring AI-driven interview systems, project evaluators, and validation agents
- Interested in practical AI tools that can be tested locally and improved into real products

## Tech Stack

<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Gemini-4285F4?style=for-the-badge&logo=google&logoColor=white)
![Google ADK](https://img.shields.io/badge/Google%20ADK-34A853?style=for-the-badge&logo=google&logoColor=white)
![MCP](https://img.shields.io/badge/MCP-0F172A?style=for-the-badge)
![AI Agents](https://img.shields.io/badge/AI%20Agents-111827?style=for-the-badge)
![LLM Apps](https://img.shields.io/badge/LLM%20Apps-7C3AED?style=for-the-badge)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-102230?style=for-the-badge)
![Data Science](https://img.shields.io/badge/Data%20Science-2563EB?style=for-the-badge)

</div>

## Current Focus

```text
Multi-Agent Workflows  ->  MCP Tool Execution  ->  LLM Orchestration
Local AI Prototypes    ->  Research Agents     ->  Validation Systems
```

I am currently exploring how multiple AI agents can work together to produce more reliable, useful, and structured outputs for real-world workflows.

## GitHub Snapshot

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=Sachiin4&show_icons=true&theme=github_dark&hide_border=true&rank_icon=github" alt="Sachin's GitHub stats" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Sachiin4&layout=compact&theme=github_dark&hide_border=true&hide=Jupyter%20Notebook" alt="Top Languages" />

</div>

---

<div align="center">

**Building AI systems that are useful, testable, and practical.**

</div>
